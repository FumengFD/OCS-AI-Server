...
if /i "%M%"=="y" (
    ...
    if errorlevel 1 echo [WARN] MinerU install failed
)
echo.
echo Vision model (press Enter to skip):
echo Example: gpt-4o  (use same API)
echo Example: deepseek-chat
set /p VM="> "
if not "%VM%"=="" echo VISION_MODEL=%VM%>> .env
echo.
...
if errorlevel 1 ( echo [INFO] MinerU not installed
) else ( echo [ OK ] MinerU OCR )
